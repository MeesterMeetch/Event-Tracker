...
describe('formatDayLabel', () => {
  ...
  it('formats other days as a short weekday + date', () => {
    vi.setSystemTime(new Date('2026-07-14T18:00:00.000Z'));
    // Jul 16, 2026 is a Thursday; exact punctuation is locale-dependent
    expect(formatDayLabel('2026-07-16')).toMatch(/^Thu,? Jul 16$/);
  });
...
describe('formatTimeOnly / formatGameTime', () => {
  ...
  it('joins date and time with a middle dot', () => {
    // Local-timezone dependent, so only assert the structure
    expect(formatGameTime('2026-07-11T23:05:00.000Z')).toMatch(/^\w{3} \d{1,2} · \d{1,2}:\d{2}/);
  });
});
