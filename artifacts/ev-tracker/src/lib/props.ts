 ...
 * devigged into fair probabilities (power method by default, see odds-math),
 * samples are combined across books quoting the exact same player and line
 * with sharp books weighted more heavily, and any side whose best available
 * price beats that consensus by at least `minEdgePercent` is returned. As
 * with game lines, at least 2 distinct books must quote a line before it
  ...
  for (const bookmaker of event.bookmakers) {
    ...
    for (const market of bookmaker.markets) {
      ...
      for (const outcomes of pairs.values()) {
        ...
        const rawProbs = outcomes.map((o) => americanToImpliedProb(o.price));
