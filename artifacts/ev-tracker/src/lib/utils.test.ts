...
/**
 * Boundary tests for the web date/format helpers. The mobile app carries a
 * ported copy of these helpers (artifacts/ev-mobile/lib/format.ts) with its
 * own mirrored suite — if either copy drifts on the Eastern-day boundaries,
 * web and mobile would bucket the same late-night game under different dates.
 * Cases here intentionally mirror artifacts/ev-mobile/lib/format.test.ts,
 * adapted to this module's API (e.g. formatPercent renders "-" for null and
 * formatOdds takes a plain number).
 */
